...
if [ $IOT_MONOLITH_VERSION = "latest" ]; then \
  sed -e "s#{IOT_MONOLITH_HOSTNAME}#iot-iot-monolith-latest.rancher-iot.lan.wurstsalat.cloud#g" "nginx_template.conf" >"nginx.conf"; else \
  sed -e "s#{IOT_MONOLITH_HOSTNAME}#iot-monolith-http.iot-iot-monolith-${IOT_MONOLITH_VERSION}.svc.cluster.local#g" "nginx_template.conf" >"nginx.conf"; fi && \
rm "${PACKAGE_BUNDLE}"
