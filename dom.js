...
async function fetchTemplate(slug) {
  ...
  const templateString = await fetch(templateUrl, { credentials: 'include', redirect: 'follow' }).then((response) => {
...
async function fetchStyle(slug) {
  ...
  const styleString = await fetch(styleUrl, { credentials: 'include', redirect: 'follow' }).then((response) => {
    if (!response.ok) return null;
    return response.text();
  }).catch(() => {
    return null;
