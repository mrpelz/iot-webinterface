...
const getTree = async (apiBaseUrl: string, interval: number, id: string) => {
  const url = new URL(TREE_URL, apiBaseUrl);
  ...
  if (tree) {
    workerConsole.debug('tree request success');
  } else {
    workerConsole.error('tree request failure');
  }
